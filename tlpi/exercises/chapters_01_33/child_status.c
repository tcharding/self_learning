...
#ifndef WCONTINUED       /* Not present on older versions of Linux */
#define WCONTINUED 0			       
#endif

static void printSigInfoFromWait(siginfo_t *si);
...
main(int argc, char *argv[])
{
	...
	default:            /* Parent: repeatedly wait on child until it
		...
		for (;;) {
			err = waitid(P_ALL, 0, &info, WEXITED | WSTOPPED | WCONTINUED);
			if (err == -1)
				errExit("waitid");
			printSigInfoFromWait(&info);
...
/* printSigInfoFromWait: prints fields filled in by waitid() */
static void
printSigInfoFromWait(siginfo_t *si)
{
	printf("si_code: %d\n", si->si_code);
	printf("si_pid: %ld\n", (long) si->si_pid);
	printf("si_signo: %d\n", si->si_signo);
