...
main(int argc, char *argv[])
{
	...
	if (fd == -1)
		errExit("cannot open server's well known file");

	nread = read(fd, &serverId, sizeof(serverId));
	if (nread != sizeof(serverId))
		errExit("read");
	...
	alarm(TIMEOUT);
	if (msgsnd(serverId, &req, REQ_MSG_SIZE, 0) == -1 && errno == EINTR)
		errExit("msgsnd timeout");
	else
		errExit("msgsnd");
	alarm(0);
