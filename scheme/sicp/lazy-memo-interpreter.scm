;;;; Upward compatible lazy evaluation with memoize
;;;;
;;;; Exercise 4.31

;; Has bugs, not being lazy
...
(define (apply-compound-procedure procedure arguments env)
  (define (list-of-processed-args formal-parameters arguments)
    (if (null? formal-parameters)
        '()
        (cons
         (let ((p (car formal-parameters))
               (a (car arguments)))
           (if (and (pair? p)
                    (pair? (cdr p))
                    (eq? (cadr p) 'lazy))
                 (delay-it a env)
                 (actual-value a env)))
         (list-of-processed-args (cdr formal-parameters) (cdr arguments)))))

  (define (parameter-names formal-parameters)
    (map (lambda (p) (if (pair? p) (car p) p)) formal-parameters))

  (eval-sequence
   (procedure-body procedure)
   (extend-environment
    (parameter-names (procedure-parameters procedure))
    (list-of-processed-args (procedure-parameters procedure)
                            arguments)
...
(define input-prompt ";;; Lazy-Memo-M-Eval input:")
(define output-prompt ";;; Lazy-Memo-Eval output:")
