 ...
 * modifications: added setopt() and included socket.h
 */
#include "socket.h"
...
/* add options here you wish to query */
struct sock_opts {
  const char	   *opt_str;
  int		opt_level;
  int		opt_name;
...
int setopt(int sockfd)
{
	...
	for (ptr = sock_opts; ptr->opt_str != NULL; ptr++) {
		...
		if (ptr->opt_val_str == NULL)
			...
			len = sizeof(val);
			setsockopt(sockfd, ptr->opt_level, ptr->opt_name,
								   &val, &len)) == -1)
		}
	}
}

void print_opt_val(int sockfd, const char *msg)
{
	socklen_t len;
	struct sock_opts *ptr;
