...
printer_thread(void *arg)
{
	...
	for (;;) {
		...
		*icp++ = TAG_OPERATION_ATTR;
		...
		if (PR_REQ_ORIENT(jp->req.flags)) {
			if (jp->req.flags & PR_ORIENT_P) 
				p = "portrait";
			else if (jp->req.flags & PR_ORIENT_RP)
				p = "reverse-portrait";
			else if (jp->req.flags & PR_ORIENT_L)
				p = "landscape";
			else if (jp->req.flags & PR_ORIENT_RL)
				p = "reverse-landscape";
			icp = add_option(icp, TAG_NATULANG /* ? */,
					 "orientation-requested", p);
		}
		if (PR_REQ_SIDES(jp->req.flags)) {
			if (jp->req.flags & PR_SIDES_ONE)
				p = "one-sided";
			else if (jp->req.flags & PR_SIDES_TWO_SHORT)
				p = "two-sided-short-edge";
			else if (jp->req.flags & PR_SIDES_TWO_LONG)
				p = "two-sided-long-edge";
			icp = add_option(icp, TAG_NATULANG /* ? */,
					 "sides", p);
		}
		*icp++ = TAG_END_OF_ATTR;
	...
	while ((nr = tread(sfd, bp, bufsz, 5)) > 0) {
		...
		if (cp == statcode) { /* Bad format; log it and move on */
			...
			*cp = '\0';
			...
			if (STATCLASS_OK(i))
				success = 1;
			if(STATCLASS_CLIERR(i) || STATCLASS_SRVERR(i)) {
				log_error(i);
			}
			break;
		}
...
/* log error code msg */
static void log_error(int err_code)
{
	struct err_info *eip;

	for (eip = err_tab; eip->err_code != STAT_INVALID; eip++) {
		if (eip->err_code == err_code) {
			log_msg("log_error: %s", eip->err_msg);
			return;
		}
	}
