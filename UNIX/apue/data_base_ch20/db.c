 ...
 */
DBHANDLE
db_open(int hashchains, const char *pathname, int oflag, ...)
{
	DB			*db;
	int			len, mode;
	size_t		i;
	...
	db->nhash = (hashchains != 0) ? hashchains : NHASH_DEF; /* hash table size */
	hash = Malloc((db->nhash + 1) * PTR_SZ + 2 ); /* +2 for newline and null */
	...
	if ((oflag & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC)) {
		...
		if (statbuff.st_size == 0) {
			/*
			 * We have to build a list of (db->nhash + 1) chain
...
/* exposed wrapper for _db_hash */
unsigned long db_hash(DBHANDLE h, const char *key)
{
	return _db_hash(h, key);
}
/*
...
_db_findfree(DB *db, int keylen, int datlen)
{
	...
	needed = (size_t)(keylen + datlen);
	while (offset != 0) {
		nextoffset = _db_readidx(db, offset);
		available = strlen(db->idxbuf) + db->datlen;
