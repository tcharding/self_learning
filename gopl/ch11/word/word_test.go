...
// randomNonPalindrome returns a word that is not a palindrome and whose
// length and contents are derived from the pseudo-random number generator rng.
func randomNonPalindrome(rng *rand.Rand) string {
	...
	for i := 0; i < (n+1)/2; i++ {
		r := rune(rng.Intn(0x1000)) // random rune up to '\u0999'
		s := rune(rng.Intn(0x1000))
		if r == s || !unicode.IsLetter(s) || !unicode.IsLetter(r) {
			i-- // rerun loop
			continue
		}
		runes[i] = r
...
func TestRandomPalindromes(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		if IsPalindrome(np) {
			t.Errorf("IsPalindrome(%q) = true count: %d\n", np, utf8.RuneCountInString(np))
		}
